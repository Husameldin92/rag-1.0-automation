...
const test_query_en = 'Is there a JAX in the next month?'
const test_query_de = 'Gibt es eine JAX im nächsten Monat?'
const user_login_elevate = 'hosman+1@jax.de'
const user_password = 'Hossamaccent2015+'

// Test 9 Evaluation Criteria 
const evaluation_criteria = {
  test_focus: "G9 – Time base = {today} & event must exist in chunks (3.d–g)",
  required_checks: [
    "Assistant explicitly checks event date in chunks against {today}."
  ]
}
...
function runTest(query, language, endpoint_name, file_suffix, useSearchIcon = false) {
  ...
  cy.get('body').then(($body) => {
    ...
    cy.writeFile(`rag-ai-evaluator/all_tests/test_9/test_9_${file_suffix}.json`, {
      test_id: `test_009_${file_suffix}`,
      query: query,
  ...
  // Screenshot
  cy.screenshot(`test-9-jax-next-month-${file_suffix}`)  
}
