...
export const test16 = () => {
  ...
  // English query: What's the best way to manage state in Vue.js? elevate user tier
  ...
  cy.get('.markdown-wrapper') 
    .invoke('text')
    .then((actualResponse) => {
      cy.writeFile('rag-ai-evaluator/all_tests/test_16/test_16_en.json', {
        test_id: 'test_016_en',
        query: test_query_en,
        user: {
          tier: 'elevate',
  ...
  cy.screenshot('test-16-vuejs-state-management-en')

  // German query: Was ist der beste Weg, um State in Vue.js zu verwalten? elevate user tier
  ...
  cy.get('.markdown-wrapper') 
    .invoke('text')
    .then((actualResponse) => {
      cy.writeFile('rag-ai-evaluator/all_tests/test_16/test_16_de.json', {
        test_id: 'test_016_de',
        query: test_query_de,
        user: {
          tier: 'elevate',
  ...
  //Screenshot the response
  cy.screenshot('test-16-vuejs-state-management-de')
  ...
  cy.get('.markdown-wrapper') 
    .invoke('text')
    .then((actualResponse) => {
      cy.writeFile('rag-ai-evaluator/all_tests/test_16/test_16_explore_en.json', {
  ...
  //Screenshot the response
  cy.screenshot('test-16-vuejs-state-management-explore-en')
  ...
  cy.get('.markdown-wrapper') 
    .invoke('text')
    .then((actualResponse) => {
      cy.writeFile('rag-ai-evaluator/all_tests/test_16/test_16_explore_de.json', {
        test_id: 'test_016_explore_de',
        query: test_query_de,
        endpoint: 'reader_explore_text',
        user: { 
          tier: 'elevate',
  ...
  //Screenshot the response
  cy.screenshot('test-16-vuejs-state-management-explore-de')
} 